...
import { Bus, Menu, X, ChevronDown, User, LogOut, BookOpen, Wallet, Bell, MapPin, Tag, HelpCircle, Phone } from 'lucide-react'
...
export default function Navbar() {
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-lg shadow-nav border-b border-slate-100' : 'bg-white border-b border-slate-100'}`}>
          ...
          <div className="flex items-center h-16 gap-8">
            ...
            <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <div className="hidden sm:block">
                <span className="font-extrabold text-lg text-slate-900 leading-none">Prayag</span>
                <span className="font-extrabold text-lg text-primary-600 leading-none">Travels</span>
              </div>
            </Link>
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `relative px-3.5 py-2 rounded-lg text-sm font-semibold transition-all duration-150 ${
                      isActive ? 'text-primary-600 bg-primary-50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                    }`
                  }
                >
                  {l.label}
              ...
              {isLoggedIn ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="hidden sm:block text-sm font-semibold text-slate-700 max-w-[100px] truncate">{user?.name?.split(' ')[0]}</span>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-glass border border-slate-100 py-1.5 z-50 animate-slide-up">
                        ...
                        {[
                          ...
                          { to: '/dashboard/wallet', icon: Wallet, label: 'Wallet & Rewards' },
                ...
                <div className="hidden sm:flex items-center gap-2">
                  <Link to="/login" className="px-4 py-2 text-sm font-semibold text-slate-700 hover:text-primary-600 transition-colors">
                    Sign In
                  </Link>
                  <Link to="/register" className="px-4 py-2 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition-colors shadow-btn">
                    Sign Up Free
                  </Link>
                </div>
              )}
          ...
          <div className="absolute right-0 top-0 h-full w-72 bg-white shadow-2xl animate-slide-up flex flex-col">
            ...
            <nav className="flex-1 py-3 px-3 overflow-y-auto">
              {navLinks.map(l => (
                ...
                >
                  {l.label}
                  {l.badge && <span className="ml-auto bg-accent-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full">{l.badge}</span>}
                </NavLink>
              ))}
              {isLoggedIn && (
                <>
